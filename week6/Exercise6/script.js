...
function parsePrice(priceText) {
    //loại bỏ chữ "VNĐ" và dấu chấm, chuyển thành số
    const cleanPrice = priceText.replace(/[^0-9]/g, '');
    return Number(cleanPrice);
}

//hàm kiểm tra sản phẩm có nằm trong khoảng giá không
...
//xử lý sự kiện tìm kiếm và lọc nâng cao
searchBtn.addEventListener('click', function() {
    //lấy giá trị từ ô tìm kiếm
    const searchTerm = searchInput.value.toLowerCase();
    
    //lấy giá trị từ dropdown lọc giá
    ...
    products.forEach(function(product) {
        ...
        //lấy giá sản phẩm
        const priceElement = product.querySelector('p strong').parentElement;
        ...
        const matchPrice = matchPriceRange(price, priceRange);
        
        //hiển thị sản phẩm nếu thỏa cả 2 điều kiện
